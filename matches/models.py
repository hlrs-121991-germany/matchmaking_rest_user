...
class Match(models.Model):
    ...
    def user_matches_update_receiver(sender, user, *args, **kwargs):
        ...
        update_top_suggestions = PositionMatch.objects.update_top_suggestions(
            user, 20)
            ...
            if job_set.count > 0:
                for job in job_set:
                    try:
                        ...
                        jobmatch, created = self.get_or_create(user=user,
                                                               job=the_job)
                    except:
                        pass
                    try:
                        the_loc = Location.objects.get(
                            name__iexact=job.location)
                        locmatch,created = LocationMatch.objects.get_or_create(
                            user=user, location=the_loc)
                    except:
                        pass
                    try:
                        the_employer = Employer.objects.get(
                            name__iexact=job.employer_name)
                        empymatch,created=EmployerMatch.objects.get_or_create(
                            user=user, employer=the_employer)
                    except:
                        pass
...
class PositionMatch(models.Model):
    ...
    def get_match_url(self):
        return reverse("position_match_view_url",
                       kwargs={"slug": self.job.slug })
...
class EmployerMatch(models.Model):
    ...
    @property
    def get_match_url(self):
        return reverse("employer_match_view_url",
                       kwargs={"slug": self.employer.slug })
...
class LocationMatch(models.Model):
    ...
    @property
    def get_match_url(self):
        return reverse("location_match_view_url",
